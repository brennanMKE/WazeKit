...
run_jazzy() {
    jazzy \
        --clean \
        --author ${ProjectName} \
        --author_url https://github.com/${GitHubPrefix}/${ProjectName} \
        --github_url https://github.com/${GitHubPrefix}/${ProjectName} \
        --module-version ${AppVersion} \
        --xcodebuild-arguments -scheme,${Target} \
        --module ${ProjectName} \
        --root-url http://cocoadocs.org/docsets/${ProjectName}/${AppVersion}/ \
...
case "${Command}" in
    ...
    pod-trunk-push)
        ...
        ;;
    *)
        echo "Usage: `basename $0` { build | clean | jazzy | setup | all | bundle-install | pod-spec-lint | pod-trunk-push }"
        ;;
esac
